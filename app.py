...
from settings import possible_emails, filename as file_name
import csv
...
def save_in_csv_question(data, file_ident='protonmail_accounts'):
    filename = file_ident + '_' + file_name
            ...
            if get_choice.is_integer(accounts_count):
                try:
                    ...
                    for _ in range(int(accounts_count)):
                        ...
                        proton_accounts.append([protonmail_login, protonmail_password])
                    ...
                    if proton_accounts:
                        ...
                        for item in proton_accounts:
                            print(f'Proton login: {item[0]}\nProton password: {item[1]}')
                        save_in_csv_question(data=proton_accounts)
                    else:
                        print("No accounts((")
            else:
            ...
            if get_choice.is_integer(accounts_count):
                try:
                    ...
                    for _ in range(int(accounts_count)):
                        ...
                        jetbrains_accounts.append([protonmail, protonmail_password, jetbrains_password])
                except Exception as exc:
                    print(exc)
                finally:
                    if jetbrains_accounts:
                        ...
                        for item in jetbrains_accounts:
                            print(f'Proton login: {item[0]}\nProton password: {item[1]}\nJetbrains password: {item[2]}')
                        save_in_csv_question(data=jetbrains_accounts, file_ident='Jetbrains_accounts')
                    else:
                        print("No accounts((")
            else:
                print("You need to input an integer!")
